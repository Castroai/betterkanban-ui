...
export const OpenCard = ({ cardId, closeModal }: { cardId: number, closeModal: () => void }) => {
    ...
    const deleteHandler = async () => {
        const confirmed = window.confirm(`Are you sure you want to delete ${state?.title}`);
        if (confirmed) {
    ...
    if (state && newState) {
        return (
            <form onSubmit={submitHandler} className="w-auto max-h-screen overflow-y-scroll h-auto rounded-md p-4  flex  flex-col gap-4 bg-light-secondary dark:bg-dark-secondary text-light-text dark:text-dark-text">
                ...
                <div className="flex h-auto" >
                    <div className="flex flex-col gap-4 ">
                        ...
                        <textarea value={newState.description} onChange={(e) => {
                            ...
                            className="border-2 border-light-primary rounded-md w-full h-96" cols={80} rows={5} />
                    </div>
                </div>
                <div className="flex justify-between items-center">
                    ...
                    <div className="cursor-pointer" onClick={deleteHandler}>DELETE</div>
