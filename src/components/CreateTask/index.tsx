...
export const CreateStory = ({ closeModal }: { closeModal: () => void }) => {
  ...
  const [prompt, setPrompt] = useState(
    "As a customer I want to create signup page so that new customer can create an account"
  );
  ...
  const placeHolderPrompt = `As a [type of user], 
    I want [a feature] 
    ...
    <div className="w-full m-10 min-w-max h-auto bg-light-secondary dark:bg-dark-secondary rounded-md">
      ...
      >
        ...
        <textarea
          ...
          className="p-2 rounded-md border-gray-200  w-full"
          value={prompt}
          onChange={(e) => {
        ...
        <div className="flex">
          <Button variant="SECONDARY" type="button" onClick={submitPrompt}>
            Generate
          </Button>
        </div>
        <hr />
        <div>
          {generating ? (
            "Generating Prompt using AI"
          ) : (
            <div className="flex flex-col">
              <span>Details</span>
              <textarea
                aria-placeholder={placeHolderPrompt}
                rows={5}
                cols={40}
                onChange={(e) => {
                  setState((current) => ({
                ...
                className="p-2 rounded-md border-gray-200  w-full h-auto"
                value={
                  (promptResponse &&
